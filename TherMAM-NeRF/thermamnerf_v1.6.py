...
for epoch in tqdm(range(START_EPOCH, CFG['n_epochs'] + 1), desc='Training Epochs'):
    ...
    cfg_step['density_scale'] = 3.0 + (10.0 - 3.0) * min(epoch / 100, 1.0)
    epoch_loss = []
    for batch in tqdm(train_dl, desc=f'Epoch {epoch}/{CFG["n_epochs"]}', leave=False):
        optimiser.zero_grad()
        mlp_1.zero_grad()
        loss_dict, _, _ = run_one_batch(batch, encoder, mlp, mlp_1, cfg_step, alpha, DEVICE0, DEVICE1)
    ...
    if epoch % 10 == 0 or epoch == 1:
        ...
        with torch.no_grad():
            for batch in val_dl:
                ld, _, _ = run_one_batch(batch, encoder, mlp, mlp_1, cfg_step, alpha, DEVICE0, DEVICE1)
...
if __name__ == '__main__':
    print('Script executed directly. Training will run on import-order execution in this script.')
