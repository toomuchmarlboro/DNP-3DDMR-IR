...
DDP usage: `torchrun --nproc_per_node=2 ThermMAM-NeRF/thermamnerf_v2.6.py`
"""
...
OUTPUT_DIR       = str(SCRIPT_DIR / 'thermamnerf_outputs2.6')
...
CFG = {
    ...
    'near'            : -1.0,
    'far'             : 1.0,
    'density_scale'   : 10.0,    # Reverted to 10.0 to prevent the 'solid block' trap
    ...
    'lr'              : 5e-4,
    'lambda_dice'     : 1.0,
    'lambda_bg'       : 2.0,     # Relaxed to allow multi-view disagreement without aggressive carving
    'lambda_thermal'  : 0.0,
...
for epoch in tqdm(range(START_EPOCH, CFG['n_epochs'] + 1), desc='Training Epochs'):
    ...
    # Fast ramp-up over 10 epochs: prevents solid block trap AND prevents collapse!
    cfg_step['lambda_bg'] = CFG.get('lambda_bg', 3.0) * min(1.0, epoch / 10.0)
...
if IS_MAIN:
    try:
        ...
        plt.suptitle('TherMAM-NeRF v2.2 Training Progress')
        plt.tight_layout()
