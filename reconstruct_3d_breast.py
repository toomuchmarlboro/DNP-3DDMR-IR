...
def orthographic_project(
    points_3d: np.ndarray,   # (N, 3) in patient-centred coords
    yaw_deg: float,
    image_h: int,
    image_w: int,
    scale: float = 1.0
    ...
    # After rotation: X→right, Y→up, Z→depth (towards camera at yaw=0)
    x_cam = rotated[:, 0]                # horizontal in image  → col
    y_cam = rotated[:, 1]                # vertical in image    → row (flipped)

    # Normalise from [-scale, +scale] → [0, W-1] / [0, H-1]
...
def perspective_project(
    ...
    focal_scale: focal_length relative to scale (larger = less distortion).
    Returns pixel coordinates (N, 2) as float32: [row, col].
    """
    R = yaw_rotation_matrix(yaw_deg)
    ...
    x_cam = rotated[:, 0]
    y_cam = rotated[:, 1]
    z_cam = rotated[:, 2]  # depth; camera is at z_cam = +focal_scale*scale

    f = focal_scale * scale
    cam_z = f + scale          # camera distance from origin
...
def voxel_carving(
    ...
    for vd in views:
        ...
        if projection == "perspective":
            ...
            pix = orthographic_project(coords, vd.yaw_deg, H, W, scale=scale)
